...
function loadPuter(): Promise<any> {
  if (typeof window === "undefined") return Promise.reject(new Error("Puter est disponible uniquement dans le navigateur."));
  const existing = (window as any).puter;
  if (existing?.ai?.chat) return Promise.resolve(existing);
  if (puterReady) return puterReady;

  puterReady = new Promise((resolve, reject) => {
    const finish = () => {
      const puter = (window as any).puter;
      if (puter?.ai?.chat) resolve(puter);
...
function installKeylessChatFetch() {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    try {
      const rawBody = typeof init?.body === "string" ? init.body : await (input instanceof Request ? input.clone().text() : Promise.resolve("{}"));
      ...
      const puterMessages = messages
        ...
        .map((message) => ({
          ...
          content: Array.isArray(message.parts)
            ? message.parts.filter((part: any) => part?.type === "text" && typeof part.text === "string").map((part: any) => part.text).join("\n")
      ...
      return new Response(JSON.stringify({ error: error instanceof Error ? error.message : "Le service IA est indisponible." }), { status: 503, headers: { "Content-Type": "application/json" } });
    }
  };
}
